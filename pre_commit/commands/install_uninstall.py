...
PRIOR_HASHES = (
    ...
    b'138fd403232d2ddd5efb44317e38bf03',
)
CURRENT_HASH = b'b5009fee27070c5fa519efefb2222a27'
...
def _hook_paths(
    ...
    hooks_base_dir = pathlib.Path(sapling.get_root()) / ".sl/pre-commit-hooks"
    return hooks_base_dir / hook_type, hooks_base_dir / f"{hook_type}.legacy"
...
def _install_hook_script(
    ...
    if os.path.lexists(hook_path) and not is_our_script(hook_path):
        ...
        raise Exception("This is not supported with Sapling.")
    
    existing_hook = sapling.get_hook(hook_type)
    if not overwrite and existing_hook is not None and existing_hook != hook_path:
        raise Exception("Hook already existing, no migration mode supported.")
    ...
    sapling.install_hook(hook_type, hook_path)
...
def _uninstall_hook_script(hook_type: str) -> None:
    ...
    sapling.install_hook(hook_type, None)
